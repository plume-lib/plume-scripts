...
for variable in GITHUB_PAT GH_TOKEN; do
  ...
  # Two requests:  the commit statuses, then the first page of check runs.
...
# Three requests:  the commit statuses, which fail and are then retried
# successfully, and the first page of check runs.
